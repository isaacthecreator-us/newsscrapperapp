...
import { 
  ...
  ChevronDown, Sparkles, X, AlertCircle, Zap
} from 'lucide-react';
...
export default function NewsScraper() {
  ...
  const performSearch = useCallback(async () => {
    ...
    setResults([]);
    setSearchMeta(null);
    setSearchProgress('Connecting to AI...');
    ...
    try {
      ...
      if (data.error) {
        if (data.setup) {
          throw new Error(`No API key configured. Get one free:\n• Groq: console.groq.com/keys\n• OpenRouter: openrouter.ai/keys`);
        }
...
<head>
  ...
  <style>
    body { font-family: system-ui, sans-serif; padding: 40px; color: #202124; max-width: 800px; margin: 0 auto; }
...
<body>
  ...
  <div class="meta">
    <p><strong>Search:</strong> ${keywords}</p>
    ${dateFrom || dateTo ? `<p><strong>Date Range:</strong> ${dateFrom || 'Any'} → ${dateTo || 'Any'}</p>` : ''}
    <p><strong>Results:</strong> ${results.length} articles</p>
  ...
  ${results.map((r, i) => `
    <div class="article">
      ...
      <div class="article-meta">
        📰 ${r.publisher || 'Unknown'} · 📅 ${r.publishedDate || 'Unknown'}${r.publishedTime ? ' · ' + r.publishedTime : ''}
      </div>
      <div class="article-summary">${r.summary || 'No summary'}</div>
    ...
    <div className="min-h-screen bg-white" style={{ fontFamily: "'Google Sans', 'Roboto', sans-serif" }}>
      ...
      <header className="border-b sticky top-0 bg-white z-50 shadow-sm" style={{ borderColor: COLORS.borderGray }}>
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span style={{ color: COLORS.lightGray, fontSize: '20px' }}>Scraper</span>
          </div>
      ...
      <main className="max-w-3xl mx-auto px-6 py-10">
        ...
        <div className="mb-8">
          ...
          >
            ...
            />
            {keywords && (
              <button onClick={clearSearch} className="p-1.5 hover:bg-gray-100 rounded-full">
            ...
            {results.length > 0 && (
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-2 px-5 py-2.5 text-sm font-medium rounded-md border"
                    ...
                    <div className="absolute top-full right-0 mt-2 bg-white rounded-lg shadow-xl border z-50 overflow-hidden" style={{ borderColor: COLORS.borderGray }}>
                      {[
                        ...
                        <button
                          ...
                          className="w-full px-4 py-3 flex items-center gap-3 text-sm hover:bg-gray-50"
                        >
            ...
            <div className="flex items-center gap-2 mb-1">
              ...
              {searchMeta.provider && (
                <span 
                  ...
                  style={{ 
                    background: `${PROVIDER_COLORS[searchMeta.provider]}15`,
                    color: PROVIDER_COLORS[searchMeta.provider]
                  }}
                >
                  <Zap size={12} />
                  {searchMeta.provider}
            ...
            {results.map((article, index) => (
              ...
              >
                  ...
                  {article.relevanceScore && (
                    <>
                      ...
                      <span
                        ...
                        style={{
                          background: article.relevanceScore >= 80 ? '#E8F5E9' : '#FFF8E1',
                          color: article.relevanceScore >= 80 ? COLORS.green : '#F9A825'
                        }}
                      >
                        {article.relevanceScore}%
                      </span>
                ...
                <a
                  ...
                  className="block text-lg mb-2 hover:underline"
        ...
        {!isLoading && results.length === 0 && !error && !keywords && (
          <div className="text-center py-20">
            <div className="w-24 h-24 mx-auto mb-6 rounded-full flex items-center justify-center" style={{ background: COLORS.bgGray }}>
              <Search size={40} color={COLORS.borderGray} />
            </div>
            <h2 className="text-xl mb-2" style={{ color: COLORS.darkGray }}>Search news with AI</h2>
            <p className="text-sm max-w-md mx-auto" style={{ color: COLORS.lightGray }}>
              Enter keywords to search news. Supports Groq, OpenAI, Together AI, and OpenRouter.
            </p>
          </div>
        )}
      </main>
      ...
      <footer className="border-t py-4 px-6 mt-auto" style={{ borderColor: COLORS.borderGray, background: COLORS.bgGray }}>
        <div className="max-w-5xl mx-auto flex justify-between items-center text-xs" style={{ color: COLORS.lightGray }}>
          <span>Supports: Groq · OpenAI · Together AI · OpenRouter</span>
